...
if (isset($datas["submit"]) && !empty($_FILES['file']['name']))  { //FILES est une superglobale qui stocke les fichiers téléchargés via le formulaire  ['name'] est le nom du fichier téléchargé
    $file = basename($_FILES['file']['name']); //basename renvoie le nom de base d'un chemin
    $path = $directory.$file;
    ...
    if(!is_dir($directory) || !is_writable($directory)) {
        ...
        if (!in_array($type, $allowTypes)) {
            ...
            if(move_uploaded_file($tempName, $path)) {
                $insert = $mysqlClient->prepare("UPDATE users SET profilePic = :imageUrl WHERE userID = :id");
